...
import { View, TouchableWithoutFeedback, ScrollView, Animated, InteractionManager, Platform } from 'react-native';
    ...
    for (let i = 0; i < this.ezswiper.count + 2; i++) {
      if (i === scrollIndex) {
        scaleArray[i].setValue(
          1 - Math.abs(currentPageFloat - scrollIndex) * (1 - this.ezswiper.cardParams.cardScale)
        );
        ...
        scaleArray[i].setValue(
          this.ezswiper.cardParams.cardScale +
            Math.abs(currentPageFloat - scrollIndex) * (1 - this.ezswiper.cardParams.cardScale)
        );
    ...
    for (let i = 0; i < count; i++) {
      ...
      views.push(
        <View key={i} style={{ flexDirection: this.ezswiper.horizontal ? 'row' : 'column' }}>
          <View
            style={{
              [this.ezswiper.horizontal ? 'width' : 'height']: margin + this.ezswiper.offset,
          ...
          >
            <Animated.View
              style={{
                backgroundColor: 'transparent',
                width: this.ezswiper.horizontal ? this.ezswiper.cardParams.cardSide : width,
                height: this.ezswiper.horizontal ? height : this.ezswiper.cardParams.cardSide,
                transform: [
                  { [this.ezswiper.horizontal ? 'scaleY' : 'scaleX']: scaleArray[i] },
                  { [this.ezswiper.horizontal ? 'translateX' : 'translateY']: translateArray[i] }
                ]
              }}
            >
